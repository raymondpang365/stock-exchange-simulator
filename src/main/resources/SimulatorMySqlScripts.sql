DROP DATABASE IF EXISTS TradingMachine;
CREATE DATABASE TradingMachine;
USE TradingMachine;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addOrder`(pId varchar(50), pSymbol varchar(5), pQuantity int, pSide varchar(4), pType varchar(20), pTimeInForce varchar(3),
	pLimitPrice decimal(10, 2), pPrice decimal(10, 2), pRejected varchar(1), pCreditCheckFailed varchar(1))
BEGIN
...
DROP procedure IF EXISTS `setCredit`;
...
BEGIN
    ...
    SET 
    id = pCounterpartyId,
    credit_limit = pCredit
    ON DUPLICATE KEY UPDATE
    credit_limit = VALUES(credit_limit) + pCredit;
    commit;
END$$
DELIMITER ;
